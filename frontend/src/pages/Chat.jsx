...
function Chat({ user, setUser }) {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/sessions.php?user_id=${user.id}`);
  ...
  const fetchMessages = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/messages.php?user1=${user.id}&user2=${partnerId}`);
      const data = await res.json();
  ...
  const sendMessage = async (e) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/messages.php`, {
      method: "POST",
