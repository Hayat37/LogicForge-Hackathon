...
function Dashboard({ user, setUser }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resSkills = await fetch(`${import.meta.env.VITE_API_URL}/skills.php`);
      ...
      try {
        const resSessions = await fetch(
          `${import.meta.env.VITE_API_URL}/sessions.php?user_id=${user.id}`
        );
  ...
  const fetchSkills = async () => {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/skills.php`);
  ...
  const searchSkills = async () => {
    const res = await fetch(
      `${import.meta.env.VITE_API_URL}/search.php?q=${search}`
    );
    const data = await res.json();
  ...
  const requestSession = async (skill_id) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/sessions.php`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
